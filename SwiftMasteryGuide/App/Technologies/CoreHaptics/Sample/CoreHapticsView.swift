...
final class CoreHapticsViewModel: ObservableObject {
    ...
    private static func dominantFrequency(from blist: UnsafePointer<AudioBufferList>, frames: CMItemCount, sampleRate: Float64) -> Float {
        let n = Int(frames)
...
final class AudioTap {
    ...
    private func currentSampleRate(from bufferList: UnsafePointer<AudioBufferList>) -> Float64? {
        ...
        let audioFormat = AudioStreamBasicDescription(
            ...
            mBytesPerPacket: audioBuffer.mDataByteSize,
            mFramesPerPacket: 1,
            mBytesPerFrame: audioBuffer.mDataByteSize,
            mChannelsPerFrame: 1,
            mBitsPerChannel: 32,
            mReserved: 0
        )
